...
void loop() {
  ...
  if ((dist_raw < _DIST_MIN)||(dist_raw > _DIST_MAX)){ // outlier -> previous value
    dist_raw = dist_prev;
  } 
  
  if((dist_raw > _DIST_MIN)&&(dist_raw < _DIST_MAX)) {    // In desired Range
  ...
  dist_prev = dist_raw; //saving value for later
